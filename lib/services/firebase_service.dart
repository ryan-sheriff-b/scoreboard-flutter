...
class FirebaseService {
  ...
  Future<List<Map<String, dynamic>>> getAllTeamsWithScores(
      {int? groupId}) async {
    final cache = FirestoreCache();
    if (cache.teams.isEmpty ||
        cache.matches.isEmpty ||
        cache.groups.isEmpty ||
        cache.members.isEmpty) {
      await cache.refresh();
    }
    final teams = groupId != null
        ? cache.teams.where((t) => t.groupId == groupId).toList()
        : cache.teams;
    final matches = cache.matches;
    final groups = cache.groups;
    final members = cache.members;
    ...
    for (var match in matches) {
      // Count all matches
      teamTotalMatches[match.team1Id] =
          (teamTotalMatches[match.team1Id] ?? 0) + 1;
      teamTotalMatches[match.team2Id] =
          (teamTotalMatches[match.team2Id] ?? 0) + 1;

      if (match.status != 'completed') continue;

      // Count completed matches
      teamCompletedMatches[match.team1Id] =
          (teamCompletedMatches[match.team1Id] ?? 0) + 1;
      teamCompletedMatches[match.team2Id] =
          (teamCompletedMatches[match.team2Id] ?? 0) + 1;

      // Goals scored
      teamGoalsScored[match.team1Id] =
          (teamGoalsScored[match.team1Id] ?? 0) + match.team1Score;
      teamGoalsScored[match.team2Id] =
          (teamGoalsScored[match.team2Id] ?? 0) + match.team2Score;

      // Match result logic
      if (match.team1Score > match.team2Score) {
        teamMatchPoints[match.team1Id] =
            (teamMatchPoints[match.team1Id] ?? 0) + 3;
        teamWins[match.team1Id] = (teamWins[match.team1Id] ?? 0) + 1;
        teamLosses[match.team2Id] = (teamLosses[match.team2Id] ?? 0) + 1;
      } else if (match.team2Score > match.team1Score) {
        teamMatchPoints[match.team2Id] =
            (teamMatchPoints[match.team2Id] ?? 0) + 3;
        teamWins[match.team2Id] = (teamWins[match.team2Id] ?? 0) + 1;
        teamLosses[match.team1Id] = (teamLosses[match.team1Id] ?? 0) + 1;
      } else {
        teamMatchPoints[match.team1Id] =
            (teamMatchPoints[match.team1Id] ?? 0) + 1;
        teamMatchPoints[match.team2Id] =
            (teamMatchPoints[match.team2Id] ?? 0) + 1;
    ...
    for (var team in teams) {
      int? teamId = team.id;
      int? groupIdValue = team.groupId;

      // Get group name
      String groupName = '';
      final group = groupIdValue != null
          ? groups.firstWhere((g) => g.id == groupIdValue,
              orElse: () => Group(
                  id: 0, name: '', description: '', createdAt: DateTime.now()))
          : Group(id: 0, name: '', description: '', createdAt: DateTime.now());
  ...
  Future<List<Match>> getMatchesByTeamId(int teamId) async {
    ...
    try {
      ...
      List<Match> team1Matches =
          _parseMatchesFromSnapshot(team1MatchesSnapshot);
      List<Match> team2Matches =
          _parseMatchesFromSnapshot(team2MatchesSnapshot);
  ...
  Future<List<Match>> getInterGroupMatches({bool completedOnly = false}) async {
    try {
      ...
      final interGroupMatches = allMatches
          .where((match) =>
    ...
    try {
      ...
      final Map<String, dynamic> matchData = {
        ...
        'completedDate': match.completedDate != null
            ? Timestamp.fromDate(match.completedDate!)
            : null,
        'createdAt': FieldValue.serverTimestamp(),
      };
      ...
      final savedDoc =
          await _firestore.collection('matches').doc(docRef.id).get();
  ...
  Future<void> updateMatch(Match match) async {
    try {
      ...
      final originalMatchData =
          matchesSnapshot.docs.first.data() as Map<String, dynamic>;
      ...
      if (team1ScoreDiff != 0) {
        await addScore(Score(
          ...
          description:
              'Match score update: ${match.team1Name} vs ${match.team2Name}',
          createdAt: DateTime.now(),
        ));
      }
      ...
      if (team2ScoreDiff != 0) {
        await addScore(Score(
          ...
          description:
              'Match score update: ${match.team2Name} vs ${match.team1Name}',
          createdAt: DateTime.now(),
        ));
      }
      ...
      if (match.status == 'completed' &&
          originalMatchData['status'] != 'completed') {
        ...
        await addScore(Score(
          ...
          description:
              'Match completed: ${team1Result} against ${match.team2Name} (${match.team1Score}-${match.team2Score})',
          createdAt: DateTime.now(),
        ));
        ...
        await addScore(Score(
          ...
          points: 0, // No additional points, just a note
          description:
              'Match completed: ${team2Result} against ${match.team1Name} (${match.team2Score}-${match.team1Score})',
          createdAt: DateTime.now(),
        ));
      }
    } catch (e) {
